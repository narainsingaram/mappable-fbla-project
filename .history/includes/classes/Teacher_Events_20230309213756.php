...
public function event_feed($title, $type, $date, $start_time, $end_time, $description, $image, $user_to) {
    ...
    if(preg_replace('/\s+/', '', $description) != ""){
        $date_added = date('Y-m-d H:i:s'); // get current date/time
        $added_by = $this->user_object->gettingUsernamepublic function event_feed($title, $type, $date, $start_time, $end_time, $description, $image, $user_to) {
    $title = strip_tags($title);
    $description = strip_tags($description);
