...
class Notify {
    ...
    public function getNotifications() {
        ...
        while($row = mysqli_fetch_array($get_notifications_query)) {
            ...
            $notification_info_query = mysqli_query($this->con,"SELECT * FROM users WHERE username='$userLoggedIn'");
            $user_data = mysqli_fetch_array($user_info_query);

            $username = $user_data['username'];

            if (isset($_POST["{$user_data['username']}_{$user_data['id']}_mark_as_read"])) {
                echo "water";
            }
                    ...
                    <a class='flex bg-slate-100 m-1' href='index.php'>
                        ...
                        <div class="tooltip tooltip-right" data-tip="Mark As Read">
                            <form class="inline" method="POST" action="index.php">
                                <button type='submit' name='{$user_data['username']}_{$user_data['id']}_mark_as_read' class='bg-emerald-200 badge w-3 text-black border-none'>
