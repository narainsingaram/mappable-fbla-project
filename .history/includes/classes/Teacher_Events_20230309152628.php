...
public function loadAttendanceTable() {
    ...
    while ($auth_rows = mysqli_fetch_array($select_events_query)) {
        ...
        if ($desc == '') {
            $desc .= 'No description provided by you';
        }
    ...
    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
      <tr class="bg-white hover:bg-gray-50 dark:bg-slate-900 dark:hover:bg-slate-800">
        <td class="h-px w-px whitespace-nowrap">
          <a class="block h-full p-6" href="#">
            <div class="flex items-center gap-x-4">
                <i class="uil uil-text-size bg-slate-200 p-2 rounded-xl text-black"></i>
              <div>
        ...
        <td class="h-px w-px whitespace-nowrap">
          <a class="block h-full p-6" href="#">
            <span class="inline-flex items-center gap-1.5 py-0.5 px-2 rounded-full text-xs font-medium bg-slate-100 text-slate-800">
...
<div class='p-6 relative rounded-2xl shadow-[rgba(7,_65,_50,_0.1)_0px_9px_50px] hover:-translate-y-1 transition ease-in'>
    ...
    public function profile_events() {
        ...
        while($profile = mysqli_fetch_array($profile_event_query)) {
            ...
            $profile_event_content .= "<div class='bg-white z-10 relative shadow-[rgba(7,_65,_50,_0.1)_0px_9px_50px] transition ease-in px-3 pb-4 pt-2 rounded-2xl my-4'>
            <div>
                <div>
                    <div class='flex align-center'> 
                    <div class='inline-flex overflow-hidden relative justify-center items-center w-12 h-12 mt-1.5 mr-2 text-xl bg-slate-300/30 rounded-full'>
    <a href='profile.php?profile_username=$username' class='font-semibold text-gray-600'>{$row['first_name'][0]}{$row['last_name'][0]}</a>
</div>
                        <ul class='mt-2'>
                            <li>
                                <h3'>
                                    <a href='profile.php?profile_username=$username'>{$row['first_name'][0]}{$row['last_name'][0]}</a>
                                <span class='bg-blue-300/20 text-blue-500 text-xs font-semibold px-2 py-1 tracking-wide rounded'>Lvl. {$row['level']} {$row['position']}</span>
                                </h3>
                            </li>
                            <li><span class='text-gray-400 text-sm'>$date_added</span>
                            </li>
