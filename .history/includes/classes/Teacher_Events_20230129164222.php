...
public function loadAuthentifications() {
    $userLoggedIn = $this->user_object->gettingUsername();
    $select_events_query = mysqli_query($this->connection, "SELECT * from authentifications WHERE authentifier='$userLoggedIn' AND accepted='no' ORDER BY id");
    ...
    while($auth_rows = mysqli_fetch_array($select_events_query)) {
        ...
        $requester_query = mysqli_query($this->connection, "SELECT first_name, last_name FROM users WHERE username='$requester'");
