...
class Spaces {
    ...
    public function createSpace($space_id, $crt_user, $connectiontent, $type, $date) {
        ...
        if (empty($space_id) || empty($crt_user) || empty($connectiontent) || empty($type)) {
            throw new Exception("All fields are requiemerald.");
        }
    ...
    public function load_current_space($space_id) {
        ...
        if (mysqli_num_rows($spaces_query) > 0) {
            while($spc_rows = mysqli_fetch_array($spaces_query)) {
                ...
                if ($sender == $userLoggedIn && $type == 'default' ) {
                    ...
                    <li class='flex justify-end my-3'>
                        <div class='bg-emerald-400 rounded-full text-white relative max-w-xl px-3 py-2'>
                ...
                ";
                    ...
                    <li class='flex justify-start my-4'>
                        <span class='my-1 relative max-w-xl px-2 mx-2 py-2 bg-emerald-100 rounded-full'>$sender_pfp</span>
                        <div class='bg-emerald-100 rounded-full my-1 relative max-w-xl px-3 py-2'>
                            <span class='block'>$connectiontent</span>
                        </div>
                    </li>
                ";
                ...
                switch($type){
                    case "new_member":
                        $spc_content .= "
                    <li class='m-auto bg-emerald-200 w-fit px-4 py-2.5 rounded-full flex'>
                        <p class='text-emerald-600 m-auto'>
                        <span class=''>
            ...
            echo '
        <div class="bg-emerald-200/50 w-1/2 m-auto px-8 py-10 rounded-2xl">
            ...
            <h1 class="text-center text-emerald-400 font-semibold py-2 text-2xl"> No Messages Yet </h1>
            <p class="text-center text-emerald-400">Looks like no one has sent a message in the space. <br> Be the first one to send a message!</p>
            ';
        }
    }
    ...
    public function load_space_menu_links() {
        ...
        while($space = mysqli_fetch_array($all_spcs_query)) {
            ...
            if (in_array($userLoggedIn, $participant_arr)) {
                if ($space_name != "") {
                    $links_menu .= "
                        <a class='grid-rows-2 bg-emerald-200/60 hover:text-emerald-600 rounded-xl m-2 px-3 py-2' href='space.php?space=$id'>$space_name</a>
                    ";
                }
            }
        }
            ...
            if (in_array($userLoggedIn, $participant_arr)) {
                ...
                <div class='bg-white rounded-xl shadow-lg p-6 mb-6'>
                    ...
                    <div class='flex justify-end'>
                        <a href='space.php?space=$id' class='btn bg-emerald-700 hover:text-white text-white border-none capitalize mx-2 my-4 rounded-full'>Go to Organization</a>
                        <form name='index.php' method='POST' class='inline'>
                            <button class='btn bg-emerald-500 hover:text-white text-white border-none capitalize mx-2 my-4 rounded-full' name='$id-spaces-leave'>Leave</button>
                        </form>
                    </div>
                </div>
                ";
                ...
                <div class='bg-white rounded-xl shadow-lg p-6 mb-6'>
                    ...
                    <div class='flex justify-end'>
                        <form name='index.php' method='POST'>
                            <button class='btn action_btn bg-emerald-500 text-white px-4 py-2 rounded-lg hover:bg-emerald-600 transition duration-300' name='$id-spaces-request'>Join</button>
                        </form>
                    </div>
                </div>
                ";
