...
public function submitEvent($title, $type, $start_time, $end_time, $description, $image, $user_to /*index.php 4-7*/) {
    ...
    $event_query = mysqli_query($this->con, "INSERT INTO events VALUES(NULL, '$title', '$type', '$start_time','$end_time', '$description', '$image', '$added_by', '$user_to', '$date_added','no', 0)");
    $id_return = mysqli_insert_id($this->con);
    }
}


public function loadEventContent() {
$userLoggedIn = $this->user_object->gettingUsername();
$event_data_query = mysqli_query($this->con, "SELECT * FROM events WHERE user_deleted='no' ORDER BY id DESC");
        ...
        <div>
            <div>
                ...
                <span class='bg-blue-100 px-2.5 py-2.5 text-2xl font-bold text-gray-700  tracking-wide rounded-full font-mono mr-3'> $first_name[0]$last_name[0] </span>
                    <ul class='mt-2'>
                        <li><h3>$first_name $last_name</h3></li>
