...
if(isset($_POST['alter_user_info'])) {
    ...
    $verify_email_usage = mysqli_query($con, "SELECT * FROM users where email='$email'");
    ...
    if($users_related == '' || $users_related == $userLoggedIn) {
        ...
        $database_alteration = mysqli_query($con, "UPDATE users SET first_name='$first_name', last_name='$last_name', email='$email', school='$school' WHERE username = '$userLoggedIn'");
    }
    else {
